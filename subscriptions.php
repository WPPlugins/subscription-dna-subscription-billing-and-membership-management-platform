...
//print_r($result);
	...
	for($i = 0; $i <count($resultRows); $i++)
	{
		...
		?>
			...
			<tr>
			<td valign="top">
			<b><?php echo $resultRow["service_name"]; ?></b><br>
			...
			<td valign="top" style="padding-left: 15px;">
			<b>Status:</b> <?php echo ($resultRow["status"]); ?><br>

			<br>
			<b>Start / Expiration Dates:</b><br>
			<?php echo substr($resultRow["subscription_date"],0,10); ?> / <?php echo substr($resultRow["expires"],0,10); ?><br>

		<?php
		if(trim($resultRow["status"])=='Active'){
			echo "<a onClick=\"if(!confirm('Are you sure you want to Unsubscribe?')) return(false);\" href='?&subId=" . $resultRow["subid"] . "&status=Unsubscribed'>Unsubscribe</a>&nbsp;| <a onClick=\"if(!confirm('Are you sure you want to Discontinue?')) return(false);\"  href='?&subId=" . $resultRow["subid"] . "&status=Discontinued'>Discontinue</a><br>";
		}else if($resultRow["status"]=='Discontinued'){
			echo "<a onClick=\"if(!confirm('Are you sure you want to Unsubscribe?')) return(false);\"  href='?&subId=" . $resultRow["subid"] . "&status=Unsubscribed'>Unsubscribe</a>&nbsp;| <a onClick=\"if(!confirm('Are you sure you want to Re-activate?')) return(false);\"  href='?&subId=" . $resultRow["subid"] . "&status=Active'>Re-activate</a><br>";
		}else if($resultRow["status"]=='Expired'){
			echo "<a onClick=\"if(!confirm('Are you sure you want to Renew?')) return(false);\" href='?&subId=" . $resultRow["subid"] . "&card_id=" . $resultRow["ccid"] . "&renew=renew&confirmation_page=0'>Renew</a><br>";
		}
?>
			<br>
